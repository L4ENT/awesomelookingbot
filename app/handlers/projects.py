...
async def cmd_projects(message: types.Message, user: User):
    ...
    await bot.send_message(admin_user.id, _("{0} {1} needs a project!").format(user_id, username))
    await message.answer(
        _("Message is received. Please wait for an asnswer."), reply_markup=default_keyboard()
    )
