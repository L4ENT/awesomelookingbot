...
class QuizQuestions:
    ...
    messages = {
        SKILLS: {
            ...
            "format": _(
                "You can send a long text with your full story, but surely in single message."
            ),
            "example": _(
                "I am a website designer. I worked in a digital agency for 2 years. I have very much projects in my suitcase) There is a portfolio [link]. I am often fond a design of fonts ..."
            ),
        },
        INTERESTS: {
            "question": _(
                "What projects and tasks are you interested in (programming, sales, marketing, turnkey projects, short tasks, etc.)?"
            ),
            "format": _(
                "Preferably detailed (thesis), because it will be easier to find the right project for you. Be sure to write in a single message."
            ),
            "example": _("Turnkey website design, mobile application design, identity, logos"),
        },
        CONGESTION: {
            "question": _(
                "How much time per week are you willing to devote to our joint projects? You can rely on the fact that the average working week is 40 hours."
            ),
...
def build_question_text(key):
    ...
    answer_format = "{0}: {1}".format(hbold(_("Answer format")), _(message_parts.get("format")))
...
async def start_quiz(message: types.Message):
    ...
    await message.answer(build_question_text(QuizQuestions.SKILLS), reply_markup=kb)
